...
// Gelen oyuncu ID'sini alın
$oyuncu_id = isset($_GET['oyuncu_id']) ? (int)$_GET['oyuncu_id'] : 0;

// Oyuncu bilgilerini veritabanından çekin
$sorgu_oyuncu = mysqli_query($conn, "SELECT * FROM oyuncular WHERE id = $oyuncu_id");
$satir_oyuncu = mysqli_fetch_array($sorgu_oyuncu);
...
// Oyuncunun oynadığı filmleri çekin ve her filme ait resimleri alın
$sorgu_filmler = mysqli_query($conn, "
  SELECT f.*, r.adi AS resim_adi
  FROM filmler AS f
  LEFT JOIN resimler AS r ON f.id = r.film_id
  INNER JOIN film_oyuncu AS fo ON f.id = fo.film_id
  WHERE fo.oyuncu_id = $oyuncu_id
...
<body class="sub_page">
  ...
  <h1><?php echo $satir_oyuncu['adi'] . " " . $satir_oyuncu['soyadi']; ?></h1>
  <div class="row">
  <div class="col-md-4">
  <img src="images/oyuncular/<?php echo $satir_oyuncu['resim']; ?>" class="oyuncu-img" alt="<?php echo $satir_oyuncu['adi']; ?>">
  </div>
  <div class="col-md-8">
  <p><strong>Biyografi:</strong> <?php echo nl2br($satir_oyuncu['ozgecmis']); ?></p>
  </div>
  </div>

  <!-- Oyuncunun Filmleri -->
  <h3 class="mt-5">Oynadığı Filmler</h3>
  <div class="row">
  ...
  <!-- Geri Dön Butonu -->
  <a href="anasayfa.php?sayfa=oyuncular" class="btn btn-primary mt-3">Geri Dön</a>
  </div>
